...
const { Connection } = require("../Db/mongo/mongo-connection");
...
async function create(user) {
   ...
   const resultado = await Connection.db.db("disneyApi").collection("users").insertOne(user);
   return resultado;
}

async function getById(id) {
   id = new ObjectId(id);
   const resultado = await Connection.db.db("disneyApi").collection("users").findOne({ _id: id });
   return resultado;
}

async function getByEmail(email) {
   const resultado = await Connection.db.db("disneyApi").collection("users").findOne({ email: email });
   return resultado;
}
